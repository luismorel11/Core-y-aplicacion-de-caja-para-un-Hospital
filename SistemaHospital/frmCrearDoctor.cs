...
{
    public partial class frmCrearDoctor : Form
    {
        public frmCrearDoctor()
        {
            InitializeComponent();
        }
        ...
        private void CargarDatos()
        {
            ...
            if (oListaDocente.Count > 0)
            {
                ...
                {
                    tabla.Rows.Add(row.IdDoctor, row.Codigo, row.DocumentoIdentidad, row.Nombres, row.Apellidos,
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            ...
            {
                IdDoctor = Convert.ToInt32(txtiddocente.Text),
            ...
            if (eseditar)
            {
                respuesta = CD_Doctor.Editar(oDocente);
            ...
            {
                respuesta = CD_Doctor.Registrar(oDocente);
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            if (dgvdata.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show("¿Desea eliminar el Docente con codigo " + dgvdata.Rows[index].Cells["Codigo"].Value.ToString() + "?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    bool respuesta = CD_Doctor.Eliminar(idDocente);
                    if (respuesta)
                    {
