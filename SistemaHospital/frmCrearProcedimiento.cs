...
{
    public partial class frmCrearProcedimiento : Form
    {
        public frmCrearProcedimiento()
        {
            InitializeComponent();
        }
        ...
        private void CargarDatos()
        {
            ...
            if (oListaCurso.Count > 0)
            {
                ...
                foreach (Procedimiento row in oListaCurso)
                {
                    tabla.Rows.Add(row.IdProcedimiento, row.Descripcion, row.Activo == true ? "Activo" : "No Activo", row.Activo);
                }
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            ...
            Procedimiento oCurso = new Procedimiento()
            {
                IdProcedimiento = Convert.ToInt32(txtidcurso.Text),
            ...
            if (eseditar)
            {
                respuesta = CD_Procedimiento.Editar(oCurso);
            ...
            {
                respuesta = CD_Procedimiento.Registrar(oCurso);
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            if (dgvdata.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show("¿Desea eliminar el Curso  '" + dgvdata.Rows[index].Cells["Descripcion"].Value.ToString() + "'?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    bool respuesta = CD_Procedimiento.Eliminar(idCurso);
                    if (respuesta)
                    {
