...
namespace SistemaHospital
{
    public partial class frmBusqueda : Form
    {
        ...
        private void BusquedaAlumno()
        {
            ...
            if (oListaAlumno != null)
            {
                ...
                if (oListaAlumno.Count > 0)
                {
                    ...
                    tabla.Columns.Add("IdPaciente", typeof(int));
                    ...
                    {
                        tabla.Rows.Add(row.IdPaciente, row.Codigo, row.Nombres, row.Apellidos, row.DocumentoIdentidad,
                            row.FechaNacimiento.Date.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture),
                             row.Sexo, row.Deuda, row.Direccion);
                    }

                    dgvdata.DataSource = tabla;

                    dgvdata.Columns["IdPaciente"].Visible = false;
                    dgvdata.Columns["Sexo"].Visible = false;
                    dgvdata.Columns["Deuda"].Visible = false;
        ...
        {
            List<Doctor> oListaDocente = CD_Doctor.Listar();
            if (oListaDocente != null)
            {
                ...
                if (oListaDocente.Count > 0)
                {
                    ...
                    foreach (Doctor row in oListaDocente)
                    {
                        tabla.Rows.Add(row.IdDoctor, row.Codigo, row.DocumentoIdentidad, row.Nombres, row.Apellidos,row.GradoEstudio);
                    }
        ...
        private void dgvdata_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvdata.Columns[e.ColumnIndex].Name == "btnSeleccionar")
            {
                if (ModeloSeleccionado == Modelo.Alumno)
                {
                    ...
                    {
                        IdPaciente = Convert.ToInt32(dgvdata.Rows[e.RowIndex].Cells["IdPaciente"].Value.ToString()),
                        ...
                        Deuda =(float)Convert.ToDecimal( dgvdata.Rows[e.RowIndex].Cells["Deuda"].Value.ToString()),
                ...
                {
                    Doctor oDocente = new Doctor()
                    {
                        IdDoctor = Convert.ToInt32(dgvdata.Rows[e.RowIndex].Cells["IdDoctor"].Value.ToString()),
                        Codigo = dgvdata.Rows[e.RowIndex].Cells["Codigo"].Value.ToString(),
                        Nombres = dgvdata.Rows[e.RowIndex].Cells["Nombres"].Value.ToString(),
                        Apellidos = dgvdata.Rows[e.RowIndex].Cells["Apellidos"].Value.ToString(),
                        DocumentoIdentidad = dgvdata.Rows[e.RowIndex].Cells["Documento Identidad"].Value.ToString(),
