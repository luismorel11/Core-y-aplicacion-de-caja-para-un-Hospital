...
{
    public partial class frmCrearArea : Form
    {
        public frmCrearArea()
        {
            InitializeComponent();
        ...
        private void CargarDatos()
        {
            ...
            if (oListaNivel.Count > 0)
            {
                ...
                tabla.Columns.Add("Descripcion Area", typeof(string));
                tabla.Columns.Add("Descripcion Horario", typeof(string));
                ...
                foreach (Area row in oListaNivel)
                {
                    tabla.Rows.Add(row.IdArea, row.oPeriodo.IdPeriodo,row.oPeriodo.Descripcion,
                        row.DescripcionArea,row.DescripcionHorario,
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            ...
            Area oNivel = new Area()
            {
                IdArea = Convert.ToInt32(txtidnivel.Text),
                oPeriodo = new Periodo()
                {
                    IdPeriodo = Convert.ToInt32(((ComboBoxItem)cboperiodo.SelectedItem).Value)
                },
                DescripcionArea = txtdescripcionnivel.Text,
                DescripcionHorario = txtdescripcionturno.Text,
            ...
            if (eseditar)
            {
                respuesta = CD_Area.Editar(oNivel);
            ...
            {
                respuesta = CD_Area.Registrar(oNivel);
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            if (dgvdata.SelectedRows.Count > 0)
            {
                ...
                txtdescripcionnivel.Text = dgvdata.Rows[index].Cells["Descripcion Area"].Value.ToString();
                txtdescripcionturno.Text = dgvdata.Rows[index].Cells["Descripcion Horario"].Value.ToString();
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            if (dgvdata.SelectedRows.Count > 0)
            {
                DataGridViewRow currentRow = dgvdata.SelectedRows[0];
                int index = currentRow.Index;
                if (MessageBox.Show("¿Desea eliminar el Area  '" + dgvdata.Rows[index].Cells["Descripcion Area"].Value.ToString() + "'?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    int idNivel = Convert.ToInt32(dgvdata.Rows[index].Cells["idNivel"].Value.ToString());

                    bool respuesta = CD_Area.Eliminar(idNivel);
                    if (respuesta)
                    {
